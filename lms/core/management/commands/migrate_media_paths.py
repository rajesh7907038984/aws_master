...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Old media path: {{old_path}}")
        self.stdout.write("New media path: {{new_path}}")
        
        if not os.path.exists(old_path):
            self.stdout.write(self.style.ERROR("Old path does not exist: {{old_path}}"))
            return
            
        if not os.path.exists(new_path):
            self.stdout.write("Creating new path: {{new_path}}")
            os.makedirs(new_path, exist_ok=True)
            
        # Migrate files
        self._migrate_files(old_path, new_path, force)
        
        self.stdout.write(self.style.SUCCESS("Successfully migrated media files to {{new_path}}"))
    ...
    def _migrate_files(self, old_path, new_path, force):
        ...
        for root, dirs, files in os.walk(old_path):
            ...
            self.stdout.write("Created directory: {{new_dir}}")
            ...
            for file in files:
                ...
                if not os.path.exists(new_file) or force:
                    ...
                    self.stdout.write("Copied file: {{old_file}} -> {{new_file}}")
                else:
                    self.stdout.write("File already exists (skipping): {{new_file}}") 