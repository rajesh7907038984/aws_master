...
def create_default_branch_for_business(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        logger.debug(f" Created default branch for business: {instance.name}")
        ...
        if DefaultAssignmentManager.is_default_business(instance.business):
            ...
            if instance.user.role == 'globaladmin':
                ...
                if not instance.user.branch:
                    ...
                    instance.user.save()
                    logger.debug(f" Assigned Global Admin {instance.user.username} to default branch: {default_branch.name}")
                elif instance.user.branch.business != instance.business:
                    # If user has a branch but it's in a different business, update it
                    instance.user.branch = default_branch
                    instance.user.save()
                    logger.debug(f" Reassigned Global Admin {instance.user.username} to default branch: {default_branch.name}")
            else:
            ...
            if branches.exists():
                ...
                if not instance.user.branch:
                    ...
                    instance.user.save()
                    logger.debug(f" Assigned {instance.user.username} to branch: {target_branch.name}")
                elif instance.user.branch.business != instance.business:
                    # If user has a branch but it's in a different business, update it
                    instance.user.branch = target_branch
                    instance.user.save()
                    logger.debug(f" Reassigned {instance.user.username} to branch: {target_branch.name}")
