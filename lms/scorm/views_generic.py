...
def scorm_view_generic(request, topic_id):
    ...
    logger.info(f"Generic: Debug - resume_needed={resume_needed}, bookmark_applied={bookmark_applied}, lesson_status={attempt.lesson_status}")
    if resume_needed and (bookmark_applied or attempt.lesson_status not in ['not_attempted', 'not attempted']):
        logger.info(f"Generic: Returning learner with progress - redirecting to content URL: {content_url}")
        return redirect(content_url)
    else:
        logger.info(f"Generic: No redirect needed - showing player template")
