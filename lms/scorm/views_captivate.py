"""
Captivate SCORM View Handler
Clean, focused implementation for Adobe Captivate packages only
"""
import logging
...
@login_required
def scorm_view_captivate(request, topic_id):
    """
    Captivate SCORM content viewer
    Handles Adobe Captivate packages with slide-based navigation
    """
    ...
    if preview_mode:
        ...
        with transaction.atomic():
            ...
            if last_attempt:
                ...
                logger.info(f"Captivate: Continuing existing attempt {attempt.attempt_number} for user {request.user.username}")
            else:
                ...
                logger.info(f"Captivate: Created new attempt {attempt.attempt_number} for user {request.user.username}")
        ...
        if has_bookmark or has_suspend_data or has_progress:
            ...
            logger.info(f"Captivate: Setting entry='resume' (bookmark={has_bookmark}, suspend_data={has_suspend_data}, progress={has_progress})")
        else:
            attempt.entry = 'ab-initio'
            logger.info(f"Captivate: Setting entry='ab-initio' (fresh start)")
    ...
    # CAPTIVATE HANDLING: Use query parameters for resume
    ...
    if resume_needed:
        ...
        logger.info(f"Captivate: Added resume parameters to content URL")
    
    # Handle bookmark/hash fragments for Captivate packages
    hash_fragment = None
    ...
    if attempt.lesson_location:
        ...
        logger.info(f"Captivate: Set location hash fragment: {hash_fragment}")
    ...
    # Case 2: Extract bookmark from suspend_data if no direct bookmark exists
        ...
        for pattern in bookmark_patterns:
            ...
            if match:
                ...
                if extracted_location:
                    ...
                    attempt.save()
                    logger.info(f"Captivate: Extracted bookmark '{extracted_location}' from suspend_data")
                    bookmark_applied = True
                    break
        ...
        if not bookmark_applied:
            ...
            attempt.save()
            logger.info(f"Captivate: Created default location '{default_slide}' based on progress {progress}%")
            bookmark_applied = True
    ...
    if resume_needed and not bookmark_applied:
        ...
        attempt.save()
        logger.info(f"Captivate: Created failsafe default location 'slide_1'")
    ...
    response = render(request, 'scorm/player_captivate.html', context)
