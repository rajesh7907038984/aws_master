...
# @receiver(post_save, sender=ScormAttempt)  # temporarily disabled
# def invalidate_gradebook_cache_on_scorm_update(sender, instance, **kwargs):
#     """
#     Clear gradebook cache when SCORM attempts are updated
#     """
#     try:
#         # Clear all gradebook caches - topics are linked to courses via M2M so clear all
#         cache.clear()
#         logger.info(f"Cleared all caches after SCORM attempt {instance.id} update")
#         
#     except Exception as e:
#         logger.error(f"Error clearing gradebook cache after SCORM update: {str(e)}")
