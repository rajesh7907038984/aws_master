...
# SCORM files go directly to SCORM Cloud - no local storage needed
...
def get_scorm_cloud_content_model() -> Any:
    """Get SCORM Cloud content model"""
    return apps.get_model('scorm_cloud', 'SCORMCloudContent')

def get_scorm_package_model() -> Any:
    """Get SCORM package model"""
    return apps.get_model('scorm_cloud', 'SCORMPackage')

def get_scorm_registration_model() -> Any:
    return apps.get_model('scorm_cloud', 'SCORMRegistration')

def get_scorm_storage() -> 'Storage':
    """SCORM files go directly to SCORM Cloud - no storage needed"""
    return None
...
class Course(models.Model):
    ...
    def delete(self, *args: Any, **kwargs: Any) -> Tuple[int, Dict[str, int]]:
        ...
        try:
            ...
            # 8. DELETE ALL SCORM CLOUD CONTENT
            try:
                from scorm_cloud.models import SCORMCloudContent
                from scorm_cloud.utils.api import get_scorm_client
                
                # Find any SCORM Cloud content directly linked to this course
                course_scorm_contents = SCORMCloudContent.objects.filter(
                    content_type='course',
                    content_id=str(self.id)
                )
                
                if course_scorm_contents.exists():
                    logger.info(f"Found {course_scorm_contents.count()} SCORM Cloud content records directly linked to course {self.id}")
                    
                    # Delete each course-level SCORM content and its package
                    for cloud_content in course_scorm_contents:
                        package = cloud_content.package
                        
                        if package and hasattr(package, 'cloud_id') and package.cloud_id:
                            try:
                                # Delete from SCORM Cloud
                                # Get branch-specific SCORM client
                                scorm_client = get_scorm_client(branch=self.branch)
                                
                                # Delete from SCORM Cloud first (with null check)
                                if scorm_client:
                                    logger.info(f"Deleting course-level SCORM Cloud package: {package.cloud_id}")
                                    try:
                                        scorm_client.delete_course(package.cloud_id)
                                        logger.info(f"Successfully deleted course-level SCORM Cloud package: {package.cloud_id}")
                                    except Exception as delete_error:
                                        logger.error(f"Error deleting SCORM Cloud package {package.cloud_id}: {str(delete_error)}")
                                    else:
                                        logger.warning(f"SCORM client not available for branch {self.branch}, skipping cloud deletion for package {package.cloud_id}")
                                    
                                    # Delete the package record
                                    package.delete()
                                    logger.info(f"Deleted course-level SCORM package record: {package.id}")
                                
                            except Exception as e:
                                logger.error(f"Error deleting course-level SCORM Cloud package {package.cloud_id}: {str(e)}")
                        
                        # Delete the cloud content record
                        cloud_content.delete()
                        logger.info(f"Deleted course-level SCORM Cloud content record: {cloud_content.id}")
                
            except ImportError:
                logger.warning("SCORM Cloud models not available - skipping course-level SCORM deletion")
            except Exception as e:
                logger.error(f"Error during course-level SCORM content deletion: {str(e)}")
...
class Topic(models.Model):
    ...
    content_file = models.FileField(
        ...
        blank=True,
        max_length=255,
        help_text="Upload file for Video, Audio, Document content (SCORM goes directly to SCORM Cloud)"
    )
    ...
    def delete(self, *args, **kwargs):
        ...
        try:
            ...
            # 7. DELETE ALL SCORM CLOUD CONTENT
            try:
                from scorm_cloud.models import SCORMCloudContent
                cloud_contents = SCORMCloudContent.objects.filter(
                    content_type='topic',
                    content_id=str(self.id)
                )
                
                if cloud_contents.exists():
                    logger.info(f"Found {cloud_contents.count()} SCORM Cloud content records to delete for topic {self.id}")
                    
                    # Import SCORM Cloud API
                    from scorm_cloud.utils.api import get_scorm_client
                    
                    # Delete each cloud content and its package
                    for cloud_content in cloud_contents:
                        # Get the package to delete its cloud content
                        package = cloud_content.package
                        
                        if package and hasattr(package, 'cloud_id') and package.cloud_id:
                            try:
                                # Get branch from topic's course for branch-specific client
                                branch = None
                                if self.section and self.section.course and self.section.course.branch:
                                    branch = self.section.course.branch
                                
                                # Delete from SCORM Cloud
                                    # Get branch-specific SCORM client
                                    scorm_client = get_scorm_client(branch=branch)
                                    
                                    # Make direct API call to delete from SCORM Cloud (with null check)
                                    if scorm_client:
                                        logger.info(f"Deleting SCORM Cloud package: {package.cloud_id}")
                                        try:
                                            scorm_client.delete_course(package.cloud_id)
                                            logger.info(f"Successfully deleted SCORM Cloud package: {package.cloud_id}")
                                        except Exception as delete_error:
                                            logger.error(f"Error deleting SCORM Cloud package {package.cloud_id}: {str(delete_error)}")
                                    else:
                                        logger.warning(f"SCORM client not available for branch {branch}, skipping cloud deletion for package {package.cloud_id}")
                                
                                # Delete the package record
                                package.delete()
                                logger.info(f"Deleted SCORM package record: {package.id}")
                                
                            except Exception as e:
                                logger.error(f"Error deleting SCORM Cloud package {package.cloud_id}: {str(e)}")
                        
                        # Now delete the cloud content record
                        cloud_content.delete()
                        logger.info(f"Deleted SCORM Cloud content record for topic: {self.id}")
                
                # Try getting any directly related SCORM content (for older content models)
                scorm_content = getattr(self, 'scorm_content', None)
                if scorm_content:
                    # Get SCORM package to delete it from the cloud
                    package = scorm_content.package if hasattr(scorm_content, 'package') else None
                    
                    # Try to remove from SCORM cloud first if API is available
                    if package and hasattr(package, 'cloud_id') and package.cloud_id:
                        try:
                            ...
                            # Delete from SCORM Cloud
                                ...
                                # Delete from SCORM Cloud (with null check)
                                if scorm_client:
                                    logger.info(f"Deleting SCORM Cloud package from scorm_content relation: {package.cloud_id}")
                                    try:
                                        scorm_client.delete_course(package.cloud_id)
                                        logger.info(f"Successfully deleted SCORM Cloud package: {package.cloud_id}")
                                    except Exception as delete_error:
                    ...
                    # Now delete the SCORM content record
                    scorm_content.delete()
                    logger.info(f"Deleted SCORM content record for topic: {self.id}")
                    
                # Check for another possible relationship name
                cloud_content = getattr(self, 'scorm_cloud_content', None)
                if cloud_content:
                    # Get SCORM package to delete it from the cloud
                    package = cloud_content.package if hasattr(cloud_content, 'package') else None
                    ...
                    if package and hasattr(package, 'cloud_id') and package.cloud_id:
                        try:
                            ...
                            # Delete from SCORM Cloud
                                ...
                                # Delete from SCORM Cloud with improved error handling (with null check)
                                if scorm_client:
                                    logger.info(f"Deleting SCORM Cloud package from cloud_content relation: {package.cloud_id}")
                                    try:
                                        deletion_result = scorm_client.delete_course(package.cloud_id)
                                        
                                        if deletion_result and isinstance(deletion_result, dict):
                                            logger.warning(f"SCORM Cloud deletion result: {deletion_result.get('message', 'Unknown status')}")
                                        else:
                                            logger.info(f"Successfully deleted SCORM Cloud package: {package.cloud_id}")
                                    except Exception as delete_error:
                                        logger.error(f"Error deleting SCORM Cloud package {package.cloud_id}: {str(delete_error)}")
                            ...
                            # Always delete the package record locally, regardless of cloud deletion status
                            package.delete()
                            logger.info(f"Deleted SCORM package record: {package.id}")
                        except Exception as e:
                            # Log the error but continue with local deletion
                            logger.error(f"Error during SCORM Cloud package deletion {package.cloud_id}: {str(e)}")
                            logger.info("Continuing with local package deletion despite cloud error")
                            try:
                                package.delete()
                                logger.info(f"Deleted SCORM package record locally: {package.id}")
                            except Exception as local_delete_error:
                                logger.error(f"Failed to delete local SCORM package record: {local_delete_error}")
                ...
                logger.error("Cannot import SCORM Cloud models - skipping SCORM deletion")
            except Exception as e:
                logger.error(f"Error during SCORM content deletion: {str(e)}")
