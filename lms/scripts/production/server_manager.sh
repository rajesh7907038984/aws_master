...
if [ -f "/home/ec2-user/lms/production.env" ]; then
    export $(cat /home/ec2-user/lms/production.env | grep -v '^#' | xargs)
fi

# Use environment variables with fallbacks
LMS_DIR="${PROJECT_ROOT:-/home/ec2-user/lms}"
LOGS_DIR="${LOGS_DIR:-/home/ec2-user/lmslogs}"
...
case "$1" in
    ...
    status)
        ...
        if lsof -Pi :8000 -sTCP:LISTEN -t >/dev/null 2>&1; then
            ...
            PID=$(lsof -ti:8000)
            echo "🔢 Process ID: $PID"
            if curl -f -s -I http://127.0.0.1:8000/ > /dev/null 2>&1; then
        ...
        PRIMARY_DOMAIN="${PRIMARY_DOMAIN:-staging.nexsy.io}"
