        ...
        try:
            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    session_obj.save()
                    logger.info(" Updated existing session {{self.session_key}}")
                    ...
                    logger.info(" Created new session {{self.session_key}}")
                
                # Verify save worked
                if Session.objects.filter(session_key=self.session_key).exists():
                    logger.info(" Session {{self.session_key}} verified in database")
                    return True
                else:
                    logger.error(" Session {{self.session_key}} not found after save")
                    return False
