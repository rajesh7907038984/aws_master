...
class ChartHttpService {
    ...
    /**
     * Get CSRF token from page with multiple fallback methods
     */
    getCSRFToken() {
        ...
        // Method 4: From global window object (set by base.html)
        if (!token && window.CSRF_TOKEN) {
            token = window.CSRF_TOKEN;
        }
        
        if (!token) {
            console.warn('CSRF token not found for chart requests');
        }
