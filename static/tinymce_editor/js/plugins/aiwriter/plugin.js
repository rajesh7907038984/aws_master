            ...
            editor.windowManager.open({
                ...
                onSubmit: function(api) {
                            ...
                            return response.text().then(text => {
                                try {
                                    ...
                                    if (response.status === 500) {
                                        ...
                                        throw new Error('Token limit exceeded. Please contact your administrator to increase the limit.');
                    ...
                    .then(data => {
                        ...
                        if (data.success === false) {
                            ...
                            if (data.token_limit_exceeded) {
                                const usagePercent = data.usage_percentage || 100;
                                const currentUsage = data.current_usage || 0;
                                const monthlyLimit = data.monthly_limit || 0;
                                
                                throw new Error(`Your AI token limit has been exceeded (${currentUsage.toLocaleString()}/${monthlyLimit.toLocaleString()} tokens used this month). Please contact your administrator to increase the limit.`);
                            }
                        ...
                        if (data.content) {
                            ...
                            let successMessage = 'AI content generated successfully!';
                            if (data.tokens_used) {
                                successMessage += ` (${data.tokens_used} tokens used)`;
                            }
                            
                            // Show usage warning if getting close to limit
                            let showUsageWarning = false;
                            if (!data.is_unlimited && data.usage_percentage) {
                                if (data.usage_percentage > 90) {
                                    successMessage += ` Warning: You've used ${data.usage_percentage}% of your monthly token limit.`;
                                    showUsageWarning = true;
                                } else if (data.usage_percentage > 75) {
                                    successMessage += ` Note: You've used ${data.usage_percentage}% of your monthly token limit.`;
                                }
                            }
                            
                            editor.notificationManager.open({
                                text: successMessage,
                                type: showUsageWarning ? 'warning' : 'success',
                                timeout: showUsageWarning ? 6000 : 3000
                            });
                    ...
                    .catch(error => {
                        ...
                        if (error.message.includes('token limit') || error.message.includes('exceeded')) {
                            errorMessage = error.message;
                            isTokenLimitError = true;
                        } else if (error.message.includes('log in') || error.message.includes('Authentication') || error.message.includes('authentication')) {
                            errorMessage = error.message; // Use the specific authentication message
                        } else if (error.message.includes('rate limit') || error.message.includes('429')) {
                            errorMessage = 'Your token limit has been exceeded. Please contact your administrator to increase the limit.';
                            isTokenLimitError = true;
                        ...
                        editor.notificationManager.open({
                            text: errorMessage,
                            type: 'error',
                            timeout: isTokenLimitError ? 10000 : 8000
                        });
                    })
                    .finally(() => {
                        api.unblock();
