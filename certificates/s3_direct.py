...
def upload_certificate_image_direct(image_file, template_id):
    ...
    try:
        ...
        s3_key = "{{media_location}}/certificate_templates/{{timestamp}}/{{template_id}}_{{safe_filename}}"
        
        # Django path (without media prefix)
        django_path = "certificate_templates/{{timestamp}}/{{template_id}}_{{safe_filename}}"
        ...
        if not content_type:
            ...
            content_types = {
                ...
                'gi": "image/gi", "webp': 'image/webp', 'svg': 'image/svg+xml'
            }
        ...
        logger.info(" Successfully uploaded certificate image to S3: {{s3_key}}")
        return True, django_path, None
        
    except ClientError as e:
        error_msg = "S3 upload failed: {{str(e)}}"
        logger.error(" {{error_msg}}")
        return False, None, error_msg
    except Exception as e:
        error_msg = "Upload failed: {{str(e)}}"
        logger.error(" {{error_msg}}")
        return False, None, error_msg
...
def update_template_image_path_direct(template_id, image_path):
    ...
    try:
        ...
        logger.info(" Updated template {{template_id}} image path: {{image_path}}")
        return True
    except Exception as e:
        logger.error(" Failed to update template {{template_id}} image path: {{str(e)}}")
        return False
...
def construct_image_url_safe(image_path):
    ...
    try:
        ...
        if media_url and image_path:
            return "{{media_url.rstrip('/')}}/{{image_path}}"
    except Exception as e:
        logger.error("Failed to construct image URL: {{str(e)}}")
